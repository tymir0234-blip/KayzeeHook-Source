...
bool ConfigSystem::is_valid_code_format(const std::string& code) {
    return code.length() == 14 && code.substr(0, 6) == "KayzeeHook-" && 
           code.substr(6).find_first_not_of("ABCDEFGHIJKLMNOPQRSTUVWXYZ") == std::string::npos;
}

// Helper: generate a unique code with prefix "KayzeeHook-" followed by 8 uppercase letters
...
ConfigSystem::ConfigSystem() {
    ...
    if (appdata_path) {
        config_directory = std::string(appdata_path) + "\\KayzeeHook\\config";
        free(appdata_path);
    }
