...
namespace crosshair {
    void render() {
        ...
        const char* txt = "KayzeeHook";
        const char* xyz = ".public";
        const char* KayzeeHookOnly = "KayzeeHook";

        float wordW = ImGui::CalcTextSize(txt).x;
        float xyzW = ImGui::CalcTextSize(xyz).x;
        float KayzeeHookOnlyW = ImGui::CalcTextSize(KayzeeHookOnly).x;
        ...
        // Draw outline for "KayzeeHook"
        for (int dx = -1; dx <= 1; ++dx) {
            for (int dy = -1; dy <= 1; ++dy) {
                if (dx || dy) {
                    dl->AddText(ImVec2(KayzeeHookPos.x + dx, KayzeeHookPos.y + dy), outlineColor, KayzeeHookOnly);
                }
            }
        }
        
        // Draw main text for "KayzeeHook"
        dl->AddText(KayzeeHookPos, colW, KayzeeHookOnly);
        
        // Draw outline for ".public"
        for (int dx = -1; dx <= 1; ++dx) {
            for (int dy = -1; dy <= 1; ++dy) {
                if (dx || dy) {
                    dl->AddText(ImVec2(KayzeeHookPos.x + KayzeeHookOnlyW + dx, KayzeeHookPos.y + dy), outlineColor, xyz);
                }
            }
        }
        ...
        dl->AddText(ImVec2(KayzeeHookPos.x + KayzeeHookOnlyW, KayzeeHookPos.y), accentFadeColor, xyz);
